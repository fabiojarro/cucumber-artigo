...
public class CotacaoSteps {
     ...
     * @param dataInicial A data no formato "MM-DD-YYYY" (ex: "11-13-2025")
     */
    @When("eu consulto a cotação da moeda {string} para a data {string}")
    public void eu_consulto_a_cotacao_da_moeda_para_a_data(String moeda, String dataInicial) {
        // OData exige aspas simples na string de data, ex: '11-13-2025'
        String endpoint = String.format("/CotacaoMoedaDia(moeda='@moeda',dataInicial='@dataInicial')?$format=json&@moeda='%s'&@dataInicial='%s'",
                moeda, dataInicial);
    ...
    public void a_resposta_deve_conter_a_data(String dataEsperada) {
        ...
        String dataCotacao = (String) cotacoes.get(0).get("DataCotacao");
        ...
        String dataPartes = partesDataHora[0]; // "13/11/2025"
        String[] partesData = dataPartes.split("/");

        // Recompõe para YYYY-MM-DD
        String dataFormatada = partesData[2] + "-" + partesData[1] + "-" + partesData[0];

        System.out.println("Data na resposta: " + dataFormatada);
        System.out.println("Data esperada: " + dataEsperada);

        Assertions.assertEquals(dataEsperada, dataFormatada,
